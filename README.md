# PowerShell Ensure Firewall Enabled Template
...
The `Ensure-Firewall-Enabled.ps1` file is the foundation for PowerShell active response scripts that verify and enforce Windows Firewall profiles (Domain, Private, Public) are enabled and logging is configured. It provides a robust framework with built-in logging, error handling, and standardized output formatting suitable for integration with security orchestration platforms, SIEM systems, and incident response workflows.
...
```powershell
.\Ensure-Firewall-Enabled.ps1 [-LogPath <string>] [-ARLog <string>]
```
...
|---------------|--------|------------------------------------------------------------------|----------------------------------------------|
| `LogPath`     | string | `$env:TEMP\EnsureFirewall-script.log`                            | Path for execution logs                      |
...
.\Ensure-Firewall-Enabled.ps1

# Custom log path
.\Ensure-Firewall-Enabled.ps1 -LogPath "C:\Logs\EnsureFirewall.log"

# Integration with OSSEC/Wazuh active response
.\Ensure-Firewall-Enabled.ps1 -ARLog "C:\ossec\active-responses.log"
```
...
**Usage**:
```powershell
Write-Log "Firewall enforcement JSON appended to C:\ossec\active-responses.log" 'INFO'
Write-Log "Domain firewall was disabled. Enabling it." 'WARN'
...
- Main action logic execution (verify/enforce firewall profiles and logging)
...
```json
{
  ...
  "enforced": [
    {
      "profile": "Domain",
      "changes": ["enabled", "log_allowed", "log_blocked", "log_path"],
      "enabled": true
    },
    {
      "profile": "Private",
...
1. Copy `Ensure-Firewall-Enabled.ps1` to your new script name
...
### Common Issues
1. **Permission Errors**: Ensure script has write access to log paths and is run as Administrator for firewall enforcement
...
```powershell
.\Ensure-Firewall-Enabled.ps1 -Verbose
```
...
Invoke-WebRequest -Uri "https://github.com/{owner}/{repo}/releases/download/v1.0.0/Ensure-Firewall-Enabled.ps1" -OutFile "Ensure-Firewall-Enabled.ps1"
```

#### Option 3: One-liner Execution
```powershell
# Execute directly from URL (use with caution)
Invoke-WebRequest -Uri "https://github.com/{owner}/{repo}/releases/download/v1.0.0/Ensure-Firewall-Enabled.ps1" | Invoke-Expression
```
